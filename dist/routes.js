...
import { UpstreamTimeoutError, callLedger, getChaosProfile, setChaosProfile, } from "./upstream.js";
 ...
 */
function replyGatewayTimeout(request, reply, error) {
    request.log.error({
...
export async function registerRoutes(app) {
    ...
    app.get("/api/orders/:orderId", {
        ...
        try {
            ...
            if (error instanceof UpstreamTimeoutError)
                return replyGatewayTimeout(request, reply, error);
            throw error;
        }
    });
    app.get("/api/slow", {
    ...
    app.register(async (admin) => {
        admin.addHook("onRequest", async (request, reply) => {
            if (isAuthorized(request))
                return;
            request.log.warn({ event: "admin.unauthorized", requestId: request.id, route: routeLabel(request) }, "rejected an unauthenticated admin request");
        ...
        admin.post("/admin/chaos", {
            ...
            const profile = setChaosProfile(request.body);
            request.log.info({ event: "chaos.updated", requestId: request.id, ...profile }, "upstream behaviour changed");
            return profile;
        });
    });
}
